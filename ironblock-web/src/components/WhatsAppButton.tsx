...
export function WhatsAppButton({ phoneNumber, message }: WhatsAppButtonProps) {
  ...
  return (
    ...
    >
      <WhatsAppIcon className="w-8 h-8 filter grayscale" />
      <span className="absolute -top-12 right-0 bg-black text-white text-sm px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
        Fale conosco
      </span>
    </Link>
  );
